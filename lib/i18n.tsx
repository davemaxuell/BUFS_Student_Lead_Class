...
export const S = {
  ...
  bp: {
    ...
    p0: { en: "Setup", ko: "준비" },
    p1: { en: "1 · Forward pass", ko: "1 · 순전파" },
    p2: { en: "2 · Compute loss", ko: "2 · 손실 계산" },
    p3: { en: "3 · Backpropagation", ko: "3 · 역전파" },
    p4: { en: "4 · Update weights", ko: "4 · 가중치 갱신" },
    n0: { en: "A training example enters. The weights still hold their current values.", ko: "학습 예시가 들어옵니다. 가중치는 현재 값을 그대로 유지합니다." },
    n1: { en: "Forward pass: each input × its weights, summed, squashed by σ — layer by layer — to produce the prediction ŷ.", ko: "순전파: 각 입력 × 가중치를 더하고 σ로 눌러 — 층마다 — 예측 ŷ를 만듭니다." },
    n2: { en: "How wrong is it? Loss = ½(ŷ − target)². The bigger the gap, the bigger the loss.", ko: "얼마나 틀렸나? 손실 = ½(ŷ − 정답)². 차이가 클수록 손실이 큽니다." },
    n3: { en: "Backpropagation: the error flows backward, and the chain rule gives each weight a gradient — how much it pushed the output wrong.", ko: "역전파: 오차가 뒤로 흐르고, 연쇄법칙으로 각 가중치의 기울기(출력을 얼마나 틀리게 했는지)를 구합니다." },
    n4: { en: "Update: every weight steps a little against its gradient: w ← w − lr·∇. Green = it just changed. Next time the loss will be a touch lower.", ko: "갱신: 모든 가중치가 기울기 반대로 조금 이동: w ← w − lr·∇. 초록 = 방금 변경됨. 다음엔 손실이 조금 더 낮아집니다." },
