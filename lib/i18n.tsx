...
export const S = {
  ...
  emb3d: {
    ...
    dragHint: { en: "Drag to rotate · +/− to zoom · X·Y·Z axes rotate with you (the 3 PCA directions) · click a word · gold lines = country↔capital", ko: "드래그=회전 · +/−=확대 · X·Y·Z 축이 함께 회전(3개의 PCA 방향) · 단어 클릭 · 금색 선=나라↔수도" },
    busy: { en: "embedding…", ko: "임베딩 중…" },
    resetView: { en: "Reset view", ko: "시점 초기화" },
  ...
  sg: {
    eyebrow: { en: "THE WHOLE TRAINING PIPELINE", ko: "학습 전체 파이프라인" },
    title: { en: "Inside one training step (skip-gram)", ko: "학습 한 단계 들여다보기 (skip-gram)" },
    intro: {
      en: "Pick a token and watch a REAL training step end to end: ID → embedding lookup → weights → scores → softmax → loss → backprop → update. The model tries to predict a nearby word from the center word, and every number nudges to do better.",
      ko: "토큰 하나를 골라 실제 학습 한 단계를 처음부터 끝까지 보세요: ID → 임베딩 조회 → 가중치 → 점수 → 소프트맥스 → 손실 → 역전파 → 갱신. 모델은 중심 단어로 주변 단어를 예측하려 하고, 모든 숫자가 조금씩 더 잘하도록 조정됩니다.",
    },
    center: { en: "Center token", ko: "중심 토큰" },
    context: { en: "true context", ko: "실제 문맥" },
    changeCtx: { en: "next context", ko: "다음 문맥" },
    exampleNote: { en: "Task: predict the context token from the center token. (Click a dot or use the dropdown to change the center.)", ko: "과제: 중심 토큰으로 문맥 토큰을 예측하기. (점을 클릭하거나 드롭다운으로 중심을 바꾸세요.)" },
    next: { en: "Next step", ko: "다음 단계" },
    play: { en: "Auto", ko: "자동" },
    pause: { en: "Pause", ko: "정지" },
    reset: { en: "Reset", ko: "초기화" },
    ...
    st2: { en: "weights", ko: "가중치" },
    st3: { en: "scores", ko: "점수" },
    st4: { en: "softmax", ko: "소프트맥스" },
    st5: { en: "loss", ko: "손실" },
    st6: { en: "backprop", ko: "역전파" },
    st7: { en: "update", ko: "갱신" },
    n0: { en: "The only input is the center token's ID — a row number. Everything below is computed from it.", ko: "입력은 중심 토큰의 ID(행 번호) 하나뿐입니다. 아래 모든 것은 이로부터 계산됩니다." },
    n1: { en: "Embedding lookup: that ID picks one row of the embedding table E — the center token's vector.", ko: "임베딩 조회: 그 ID가 임베딩 표 E의 한 행 — 중심 토큰의 벡터 — 를 고릅니다." },
    n2: { en: "Output weights W: a second table, one weight vector per token. These score how likely each token is to appear near the center.", ko: "출력 가중치 W: 두 번째 표로, 토큰마다 가중치 벡터가 하나씩. 각 토큰이 중심 근처에 나올 가능성을 점수로 매깁니다." },
    n3: { en: "Scores (logits): the center's vector · every token's weight vector (a dot product). Bigger = the model thinks it fits better.", ko: "점수(로짓): 중심 벡터 · 각 토큰의 가중치 벡터(내적). 클수록 모델이 더 잘 맞는다고 봅니다." },
    n4: { en: "Softmax: squash the scores into probabilities that add up to 1 — the model's guess for the nearby word.", ko: "소프트맥스: 점수를 합이 1인 확률로 바꿉니다 — 주변 단어에 대한 모델의 추측." },
    n5: { en: "Loss: we wanted the TRUE context word to get high probability. Loss = −log(its probability) — low probability means big loss.", ko: "손실: 실제 문맥 단어가 높은 확률을 받길 원했습니다. 손실 = −log(그 확률) — 확률이 낮을수록 손실이 큽니다." },
    n6: { en: "Backpropagation: the gradient says push the true word's score UP and the others DOWN — and which way to move the center's embedding.", ko: "역전파: 기울기는 실제 단어의 점수는 올리고 나머지는 내리라고 — 그리고 중심 임베딩을 어느 방향으로 옮길지 — 알려줍니다." },
    n7: { en: "Update: take a small step against the gradient. Both the embedding AND the weights change, and this example's loss drops.", ko: "갱신: 기울기 반대로 조금 이동. 임베딩과 가중치가 모두 바뀌고, 이 예시의 손실이 줄어듭니다." },
    scoresFormula: { en: "score_k = E[center] · W[k]  (dot product, per token)", ko: "점수_k = E[중심] · W[k]  (토큰마다 내적)" },
    backpropNote: { en: "gradient on each score = probability − target  (target is 1 for the true word, 0 otherwise)", ko: "각 점수의 기울기 = 확률 − 목표  (목표는 실제 단어만 1, 나머지는 0)" },
    updateNote: { en: "The output weights W were nudged too — every number in the network moved a little to lower the loss.", ko: "출력 가중치 W도 함께 조정됩니다 — 네트워크의 모든 숫자가 손실을 낮추려 조금씩 이동합니다." },
    plotTitle: { en: "Input embeddings E (the center moves)", ko: "입력 임베딩 E (중심이 이동)" },
    plotNote: { en: "White = center, green = true context. Click a dot to make it the center, then step and watch its vector slide toward its context.", ko: "흰색=중심, 초록=실제 문맥. 점을 클릭해 중심으로 만들고, 단계를 진행하며 벡터가 문맥 쪽으로 이동하는 것을 보세요." },
    bridge: { en: "Run this for every token and context, millions of times, and the embedding table fills with meaning. A real encoder (BERT) does this same predict → loss → backprop loop — just with a much bigger network and the masking task.", ko: "이것을 모든 토큰·문맥에 대해 수백만 번 반복하면 임베딩 표가 의미로 채워집니다. 실제 인코더(BERT)도 같은 예측 → 손실 → 역전파 루프를 — 훨씬 큰 네트워크와 마스킹 과제로 — 수행합니다." },
  },
  ...
  tax: {
    ...
    cost4: { en: "Fairness — speakers of low-resource languages pay more for worse AI", ko: "공정성 — 저자원 언어 사용자는 더 내고 더 나쁜 AI를 받음" },
    fix: {
