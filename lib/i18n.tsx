...
export const S = {
  ...
  bp: {
    ...
    records: { en: "Error value records", ko: "오차 값 기록" },
    colIter: { en: "iter", ko: "반복" },
    colErr: { en: "error (loss)", ko: "오차 (손실)" },
  ...
  emblearn: {
    ...
    pull: { en: "pull", ko: "당김" },
    push: { en: "push", ko: "밀어냄" },
    encoderBridge: { en: "This is the classic word2vec idea — the simplest way to learn embeddings. Modern models (BERT, GPT) don't use word2vec itself: they learn an embedding table like this INSIDE the Transformer, trained by the masking/prediction task from the Encoder section. Same principle — tokens become learned vectors via prediction + backprop — just a much bigger network, and the vectors become context-aware.", ko: "이것이 고전적인 word2vec 아이디어 — 임베딩을 배우는 가장 단순한 방법입니다. 최신 모델(BERT, GPT)은 word2vec 자체를 쓰지 않습니다: 인코더 섹션의 마스킹·예측 과제로 트랜스포머 '내부'에서 이런 임베딩 표를 학습합니다. 원리는 같고(토큰이 예측과 역전파로 학습된 벡터가 됨) 네트워크가 훨씬 크며, 벡터가 문맥에 따라 달라집니다." },
